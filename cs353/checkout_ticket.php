<?php
    ...
    $ticket_id = mysqli_real_escape_string($db, $_GET['id']);
    $ticket_sql = "SELECT ticket_type, ticket_price FROM ticket WHERE ticket_id = '$ticket_id' ";
    $res_ticket = mysqli_query($db, $ticket_sql);
    $first_row = $res_ticket->fetch_assoc();
    $ticket_type = $first_row['ticket_type'];
    $ticket_price = $first_row['ticket_price'];
    ...
    if (isset($_POST['add_balance'])) 
    {
        ...
        header("location: checkout_ticket.php?id=" . $ticket_id );
    }

    if (isset($_POST['confirm_purch'])) 
    {
        if( $ticket_price > $original_balance )
        {
            $error = "Not enough balance.";
        }
        else
        {
            $sql = "UPDATE civilian SET balance = balance - '$ticket_price' WHERE user_id = '$glob_name'";
            $res = mysqli_query($db, $sql);

            $sql = "INSERT INTO buy_ticket( user_id, ticket_id )
                    VALUES ( '$glob_name', '$ticket_id' )";
            $res = mysqli_query($db, $sql);
            header("location: ticketlist.php?id=" . $ticket_id );
        }
    }
?>
        ...
        <div class="container">
            <div class = "offset-md-0 col-md-0">
                ...
                <form name="eventform" action="" method="post">
                    ...
                    <div style = "font-size:15px; color:black; margin-top:10px; text-align:center"><b>Ticket Type: <?=$ticket_type;?></b></div>
                    <div style = "font-size:15px; color:black; margin-top:10px; text-align:center"><b>Price: $<?=$ticket_price;?></b></div>
                    <br>
                    <div class="form-group">
                        <div align="center">
