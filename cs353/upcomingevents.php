<?php
    ...
    if(isset($_POST['drop_event']))
    {
        $event_id = mysqli_real_escape_string($db, $_POST['drop_event']);
        $glob_name = $_SESSION['user_id'];

        $query = "DELETE FROM joins WHERE event_id ='$event_id' and user_id = '$glob_name'";
...
<html lang="en">
    <head>
        ...
        <script>
            ...
            {
                alert("Selected event has been dropped");
            }
        </script>
            ...
            <div>
                ...
                <table class="table table-striped align-middle">
                    ...
                    <tbody>
                        <?php
                        $glob_name = $_SESSION['user_id'];
                        $sql = "(SELECT DISTINCT E.event_id, E.event_name, E.description, E.address, 
                            E.start_time, E.event_date, E.start_time, E.end_time, E.type, E.age_restriction, 
                            E.attendance, E.quota FROM any_event E, joins J WHERE organiser <> '$glob_name' and 
                            event_date > CAST(CURRENT_TIMESTAMP AS DATE) and J.event_id = E.event_id and 
                            J.user_id = '$glob_name')";
                        ...
                        while($row = $res-> fetch_assoc())
                        {
                            ...
                            <tr>
                                ...
                                <td><?= $row["quota"]; ?></td>
                                <td>
                                    <a href ="giftshop.php?id=<?=$row["event_id"];?>" class="btn btn-primary btn">Gift Shop</a>
                                    <a href ="tickets.php?id=<?=$row["event_id"];?>" class="btn btn-success">Tickets</a>

                                    <form method ="POST" class="d-inline" onsubmit="return validateForm()">
                                        <button type="submit" name="drop_event" value="<?=$row['event_id'];?>" class="btn btn-danger">Drop</button>
                                    </form>
                                </td>
                            </tr>
                            <?php
